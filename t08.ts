...
async function dblclickListener(this: HTMLElement, ev: MouseEvent): Promise<void> {
    ...
    if (list) {
        ...
        while (true) {
            await promise;
            const text = prompt(
                "Enter color code or color name:",
                "#" + commonColors[Math.floor(Math.random() * 16)].toString(16).padStart(6, "0")
            )
            if (!text) break;
            promise = new Promise(resolve => {
                const node = li.cloneNode() as typeof li;
...
let data: Readonly<Record<string, any>> = {
    "Fish": {
        "trout": {},
        "salmon": {}
    },
...
function createTree(entry: [string, Record<string, any>], container: HTMLUListElement): void {
