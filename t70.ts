...
type x2 = Omit<MessagePort, keyof Window>; // start
...
(window as any)["A1"] = "aa-window-" + Date.now();
...
function onIFrameLoad(this: HTMLIFrameElement, ev: Event) {
    // Transfer port2 to the iframe
    const contentWindow = f1.contentWindow;
    if (contentWindow) {
        (contentWindow as any)["A1"] = "aa-contentWindow-" + Date.now();
