class Uploader {
  ...
  private async getUploadedBytes(): Promise<number> {
    const url: URL = new URL("/uploadedBytes", "https://localhost:7072");
  ...
  private async uploadFetch(): Promise<string> {
    const url: URL = new URL("/uploadFetch", "https://localhost:7072");
    console.log("/uploadFetch.url:", url);

    const blob: Blob = this.file;
