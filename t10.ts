...
function clockStart(_this?: HTMLElement): void {
    if (timerId)
        return;
...
function clockStop(_this: HTMLElement): void {
    if (_this instanceof HTMLInputElement) {
...
function sortPeople(sortBy: string = "age") {
    ...
    if (liOne && sortableTable instanceof HTMLTableElement) {
        ...
        if (tBody) {
            ...
            type TPerson = { name: string; surname: string; age: string };
            ...
            const byName = (a: TPerson, b: TPerson) => collator.compare(a.name, b.name);
            const bySurname = (a: TPerson, b: TPerson) => collator.compare(a.surname, b.surname);

            const byAge = (a: TPerson, b: TPerson) => +a.age - +b.age;
