...
const tHeadRow: HTMLTableRowElement = tHeadRows[0];
...
if (!(
    permittedColumnTypes.includes(firstColumnType) &&
    permittedColumnTypes.includes(secondColumnType)
))
    throw Error("Invalid column types.");

const tdsArray: HTMLTableSectionElement[] = Array.from(grid.tBodies);
const rawRows: HTMLTableRowElement[] = tdsArray.flatMap(section => Array.from(section.rows));

const dataRows = rawRows.map(row => {
    const dataCells: HTMLTableCellElement[] = Array.from(row.cells);
    ...
    return [
        ...
        row
    ] as [string, string, HTMLTableRowElement];
});
...
grid.addEventListener("click", function (ev: MouseEvent) {

    if (!(ev.target && ev.target instanceof HTMLElement))
        return;

    const sortNumbers = (a: string, b: string) => {
        return +a - +b;
    };

    const collator = new Intl.Collator();
    const sortTexts = (a: string, b: string) => {
    ...
    if ((buttons as EventTarget[]).includes(target)) {
        const sortByType = target.dataset["type"];
        const idx: 0 | 1 = (
            buttons.findIndex(button => button === target) % 2
        ) === 1 ? 1 : 0;
