...
canvasElem.onmousemove = function (e: MouseEvent) {
...
btnSubmit.addEventListener("click", async function (this: HTMLButtonElement, ev: MouseEvent): Promise<void> {
    const blob = await new Promise<Blob | null>(resolve => canvasElem.toBlob(resolve));
...
btnGetUsers.addEventListener("click", async function (this: HTMLButtonElement, ev: MouseEvent): Promise<void> {
